...
app.post('/api/chat', auth, async (req, res) => {
  console.log('chat called, user:', req.user?.id, 'key exists:', !!ANTHROPIC_KEY);
  ...
  try {
    const langInst = req.body.lang_instruction || '';
    const sysPrompt = (system || getSystem()) + (langInst ? '\n\nחשוב: ' + langInst : '');
    const r = await fetch('https://api.anthropic.com/v1/messages', {
      method: 'POST',
      headers: { 'Content-Type':'application/json', 'x-api-key':ANTHROPIC_KEY, 'anthropic-version':'2023-06-01' },
      body: JSON.stringify({ model:'claude-haiku-4-5-20251001', max_tokens:2000, system:sysPrompt, messages:(messages||[]).slice(-16) })
    });
...
app.post('/api/analyze', auth, async (req, res) => {
  console.log('analyze called, user:', req.user?.id, 'key exists:', !!ANTHROPIC_KEY, 'key prefix:', ANTHROPIC_KEY?.substring(0,10));
  if (!ANTHROPIC_KEY) return res.status(500).json({ error: 'API Key לא מוגדר בשרת' });
  const { messages, conversation_id, lang_instruction } = req.body;
  const profile = db.prepare('SELECT * FROM profiles WHERE user_id=?').get(req.user.id);
  const profStr = profile ? `פרופיל: חוזה ${profile.contract||'?'}, ותק ${profile.seniority||'?'} שנים` : '';
  const langStr = lang_instruction || 'ענה בעברית בלבד.';
  try {
    const r = await fetch('https://api.anthropic.com/v1/messages', {
      method:'POST',
      headers:{'Content-Type':'application/json','x-api-key':ANTHROPIC_KEY,'anthropic-version':'2023-06-01'},
...
נתח את התלוש: זהה חוזה/ותק/שכר, בדוק כל רכיב עם ✅/⚠️/❌, חשב פוטנציאל גבייה, תן צעדים לתיקון. ${langStr}`,
...
app.listen(PORT, () => {
  console.log(`✅ תלוש חכם רץ על פורט ${PORT}`);
  console.log(`🔑 API Key: ${ANTHROPIC_KEY ? ANTHROPIC_KEY.substring(0,15)+'...' : '✗ חסר!'}`);
  console.log(`🔐 JWT Secret: ${JWT_SECRET ? '✓ מוגדר' : '✗ חסר!'}`);
});
